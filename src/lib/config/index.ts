import { SiteConfig, NavigationItem, SocialMedia, ContactInfo } from '@/types/config';
...
// Получить навигацию
export async function getNavigation(type: 'main' | 'footer' = 'main'): Promise<NavigationItem[]> {
...
export async function getSiteInfo(): Promise<{ siteName: string; siteDescription: string }> {
  const config = await getSiteConfig();
  return {
