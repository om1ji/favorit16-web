...
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (
      ...
      if (refreshToken) {
        try {
          console.log('Attempting to refresh token with retry count:', originalRequest._retryCount);
          ...
          const isBackgroundRequest = originalRequest?.url?.includes('/me/') || 
...
export const clearCache = (type?: 'categories' | 'brands' | 'products' | 'productDetails') => {
  if (!type) {
    // Очищаем весь кеш
